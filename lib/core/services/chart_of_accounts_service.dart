...
class ChartOfAccountsService {
  Future<void> insertFullChartOfAccounts(Database db) async {
    final accounts = PlanParser.parse(syscohadaCsv);
    final batch = db.batch();

    // Clear existing accounts to avoid duplicates/conflicts during full seed
    // Use with caution if user data is mixed, but for reference chart it's usually fine to reset on upgrade
    // or we could use INSERT OR IGNORE
    // For now, we assume this is run on a fresh migration or we want to overwrite standard accounts.
    // batch.rawDelete('DELETE FROM comptes_comptables');
  ...
  static const String syscohadaCsv = '''
10  ; CAPITAL  
101 ; CAPITAL SOCIAL  
...
7521; Quote-part transférée de bénéfices (comptabilité du gérant) 
